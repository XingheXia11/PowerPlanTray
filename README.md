...
- **一键切换**：常驻托盘，左键弹出菜单，点哪个切哪个，当前计划带 ✓；双击图标直达电源选项
- **iOS 风格菜单**：固定浅色外观——分组白卡、行间发丝分隔线、右侧对勾、圆角与柔和投影
...
- **贴细节**：原生圆角+投影（Win11）、资源管理器重启图标自动恢复、防重复启动、高 DPI 适配、欢迎气泡仅首次运行弹一次
...
运行 `PowerPlanTray.exe /export-icons` 可重新导出三档 256px 图标到 `assets/`（README 配图）。
